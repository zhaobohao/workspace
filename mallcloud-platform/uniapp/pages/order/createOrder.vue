<template>
	<view>
		...
		<view class="hot-floor" v-if="item1.giftsList && item1.giftsList.length > 0" v-for="(item1, index1) in basicGiftsList"
		 :key="index1">
		...
		<!-- 优惠明细 -->
		<view class="yt-list">
			<view class="yt-list-cell b-b" v-if="couponList && couponList.length > 0" @click="toggleMask('show')">
...
<script>
	...
	export default {
		...
		async onLoad(option) {
			...
			} else if (option.jifenType) { // 积分兑换
				this.jifenType=option.jifenType
				this.cartId = option.id;
				this.goodsId = option.goodId;
				let params = {
					cartId: option.id,
					type: option.type
				};
			...
			if (!data1 || !data1.cartPromotionItemList) {
				uni.switchTab({
			...
			this.cartPromotionItemList = data1.cartPromotionItemList;

			this.basicGiftsList = data1.basicGiftsList;
			...
			this.calcAmount = data1.calcAmount;
			this.addressData = data1.address;
			if (this.addressData) {
				this.addressId = this.addressData.id;
			}

			this.couponList = data1.couponHistoryDetailList;
			this.memberReceiveAddressList = data1.memberReceiveAddressListaddress;
		...
		methods: {
			...
			async submit() {
				...
				if (data && data.data.order) {
					let id = data.data.order.id;
					//   let url = `/pages/money/pay?id=${id}`;
					let url = '/pages/order/payment/index?order_id=' + id + '&type=1';
					uni.navigateTo({
						url: url
					});
				} else {
