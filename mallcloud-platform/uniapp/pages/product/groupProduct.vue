		...
		<view class="c-list">
			...
			<view class="eva-section">
				<view class="e-header" v-for="(item, index) in groupMemberList" :key="index">
					<img v-for="(item1, index1) in item.pics" :key="index1" :src="item1" width="30upx" height="30upx" alt="" />
					<div class="time">倒计时：{{ item.djs }}</div>

					<button type="primary" class=" action-btn no-border add-cart-btn" @click="acceptGroup(item)">参与拼团</button>
				</view>
			</view>
			<view class="c-row b-b">
	...
	async onLoad(ops) {
		...
		if (id) {
			...
			if (data) {
				...
				setInterval(() => {
				if(data.memberGroupList){
				data.memberGroupList.map((obj, index) => {
                						this.$set(obj, 'djs', this.InitTime(obj.exipreTime));
                					});

                					this.groupMemberList = data.data.memberGroupList;
				}
		...
		async addGroup(item) {
			...
			if (userInfo && userInfo.id) {
				...
				if (this.sku && this.sku.id) {
					...
					uni.navigateTo({
						url: `/pages/order/createOrder?groupId=${this.group.id}&&id=${item.id}&&groupType=1&&type=1&&skuId=${this.sku.id}&&memberId=${userInfo.id}`
					});
				} else {
					...
					uni.navigateTo({
						url: `/pages/order/createOrder?groupId=${this.group.id}&&id=${item.id}&&groupType=1&&type=1&&memberId=${userInfo.id}`
					});
				}
			}else{
				let url = '/pages/public/login';
		...
		async acceptGroup(item) {
			...
			if (userInfo && userInfo.id) {
				...
				if (this.sku && this.sku.id) {
					...
					uni.navigateTo({
						url: `/pages/order/createOrder?groupId=${item.groupId}&&mgid=${item.id}&&id=${item.goodsId}&&groupType=2&&type=1&&skuId=${this.sku.id}&&memberId=${
								userInfo.id
								}`
					});
				} else {
					...
					uni.navigateTo({
						url: `/pages/order/createOrder?groupId=${item.groupId}&&mgid=${item.id}&&id=${item.goodsId}&&groupType=2&&type=1&&memberId=${userInfo.id}`
					});
				}
			}else{
				let url = '/pages/public/login';
