	...
	<view class="container">
		<view class="carousel">
			<view class="image-wrapper">
				<image :src="detailData.icon" class="loaded" mode="aspectFill"></image>
			</view>
		</view>
		...
		async onLoad(ops) {
			...
			if (id) {
				...
				if (data) {
					...
					this.desc = detailData.content||'<p>图文详细<img class="wscnph" src="http://yjlive160322.oss-cn-beijing.aliyuncs.com/mall/images/20191023/gd.jpg" /></p>';
			...
			async jifenPay(item) {
				...
				if (userInfo && userInfo.id) {
					...
					if (this.sku && this.sku.id) {
						...
						if(this.detailData.stock<1){
							uni.showToast({title:"库存不够！"});
							return;
						}
						let params = { goodsId: id, memberId: userInfo.id,channel: 'jifen' };
						data = await Api.apiCall('post', Api.order.addCart, params);
						data =data.data;
						uni.navigateTo({
							url: `/pages/order/createOrder?id=${data.id}&goodId=${id}&type=1&memberId=${userInfo.id}&jifenType=1`
						});
					}
				}else{
					let url = '/pages/public/login';
