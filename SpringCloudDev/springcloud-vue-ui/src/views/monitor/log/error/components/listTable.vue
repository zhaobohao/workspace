<template>
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="请求数据" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.params }}</span>
        </template>
      </el-table-column>
      <el-table-column label="请求地址" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.requestUri }}</span>
        </template>
      </el-table-column>
      <el-table-column label="错误报文" min-width="880px">
        <template slot-scope="scope">
          <span class="lowCase">{{ scope.row.stackTrace }}</span>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getList(listQuery) {
        ...
        getErrorList(this.listQuery.current, this.listQuery.size, this.listQuery.query).then(response => {
