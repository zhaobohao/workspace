<template>
  <el-card>
    ...
    <el-button v-waves v-permission="['develop.dbinstance.add']" class="filter-item" style="margin-left: 10px;" round type="primary"
      icon="el-icon-edit" @click="handleCreateAction">
      {{ $t('table.add') }}</el-button>
    <el-button v-waves v-permission="['api.springcloud-develop.dbinstance.remove']" class="filter-item" style="margin-left: 10px;" round type="danger"
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="数据库名称" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" @click="handleUpdate(scope.row)">{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="data账号" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.dataUser }}</span>
        </template>
      </el-table-column>
      <el-table-column label="etl账号" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.etlUser }}</span>
        </template>
      </el-table-column>
      <el-table-column label="opr账号" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.oprUser }}</span>
        </template>
      </el-table-column>
      <el-table-column label="rpt账号" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.rptUser }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.actions')" fixed="right" align="center" width="180"
        ...
        <template slot-scope="scope">
          <el-button v-waves v-permission="['develop.dbinstance.edit']" type="primary" size="mini" @click="handleUpdate(scope.row)">
            {{ $t('table.edit') }}
          </el-button>
          <el-button v-if="scope.row.status!='deleted'" v-permission="['api.springcloud-develop.dbinstance.remove']" v-waves size="mini" type="danger"
    ...
    methods: {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['id', '数据库名称', 'data账号', 'etl账号', 'opr账号', 'rpt账号']
          const filterVal = ['id', 'name', 'dataUser', 'etlUser', 'oprUser', 'rptUser']
          const data = this.formatJson(filterVal, this.list)
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: 'datainstance-list'
          })
