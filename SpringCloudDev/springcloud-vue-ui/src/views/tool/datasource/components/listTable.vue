<template>
  <el-card>
    <el-button v-waves v-permission="['tool.datasource.add']" class="filter-item" style="margin-left: 10px;" round type="alert"
      icon="el-icon-search" @click="handleIsSearchCardShow">
      {{ $t('table.fliter') }}</el-button>
    <el-button v-waves v-permission="['api.springcloud-develop.datasource.remove']" class="filter-item" style="margin-left: 10px;" round type="primary"
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="驱动类" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.driverClass }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="密码" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.password }}</span>
        </template>
      </el-table-column>
      <el-table-column label="连接地址" width="380px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.url }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.actions')" fixed="right" align="center" width="180"
        ...
        <template slot-scope="scope">
          <el-button v-waves v-permission="['tool.datasource.edit']" type="primary" size="mini" @click="handleUpdate(scope.row)">
            {{ $t('table.edit') }}
          </el-button>
          <el-button v-if="scope.row.status!='deleted'" v-permission="['api.springcloud-develop.datasource.remove']" v-waves size="mini" type="danger"
