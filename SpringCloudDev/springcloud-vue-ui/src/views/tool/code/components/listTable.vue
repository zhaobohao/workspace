      ...
      <el-table-column label="数据源" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type"
            @click="handleUpdate(scope.row)">{{ scope.row.datasourceId | translateVlaue2Lable(dicData) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="模块名" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.codeName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="服务名" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.serviceName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="表名" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.tableName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="包名" width="380px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.packageName }}</span>
        </template>
      </el-table-column>
    ...
    methods: {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['id', '数据源', '模块名', '服务名', '表名', '表前缀', '主键名', '包名', '基础业务', '包装器', '后端生成路径', '前端生成路径']
          const filterVal = ['id', 'datasourceId', 'codeName', 'serviceName', 'tableName', 'tablePrefix', 'pkName',
            'packageName', 'baseMode', 'wrapMode', 'apiPath', 'webPath'
          ]
