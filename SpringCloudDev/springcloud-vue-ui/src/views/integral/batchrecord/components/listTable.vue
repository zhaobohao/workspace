<template>
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="批量编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.batchId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="批量所属类型" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.batchType }}</span>
        </template>
      </el-table-column>
      <el-table-column label="批量描述" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.batchDesc }}</span>
        </template>
      </el-table-column>
      <el-table-column label="批量数量" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.batchNm }}</span>
        </template>
      </el-table-column>
