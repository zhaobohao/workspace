<template>
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="表id编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actPrmId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动类别编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.code }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动类别名称" min-width="150px">
        <template slot-scope="scope">
      ...
      <el-table-column label="备注" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.apDesc }}</span>
        </template>
      </el-table-column>
      <el-table-column label="状态取自参数表" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.stat }}</span>
        </template>
      </el-table-column>
      <el-table-column label="显示顺序" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.sort }}</span>
        </template>
      </el-table-column>
      <el-table-column label="" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.hideDeptId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.dataModeId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.parentCoderId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.selectorModeId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.conditionTypeId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="所属系统ID" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.sysId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="预留字段1" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.reserveCokumn1 }}</span>
        </template>
      </el-table-column>
      <el-table-column label="预留字段2" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.reserveCokumn2 }}</span>
        </template>
      </el-table-column>
      <el-table-column label="预留字段3" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.reserveCokumn3 }}</span>
        </template>
      </el-table-column>
      <el-table-column label="日期类型（当DATA_MODE为DATE时的扩展）" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.dateType }}</span>
        </template>
      </el-table-column>
      <el-table-column label="试算是否显示字段，0显示1不显示" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.trialShow }}</span>
        </template>
      </el-table-column>
      <el-table-column label="隐藏预留字段" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.parentValue }}</span>
        </template>
      </el-table-column>
      <el-table-column label="转换表达式" min-width="150px">
        <template slot-scope="scope">
