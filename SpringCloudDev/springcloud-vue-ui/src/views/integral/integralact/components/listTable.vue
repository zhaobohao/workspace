  ...
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralActId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="积分活动编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="积分活动名称" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="营销活动编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.marketActId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="营销活动结束时间" min-width="150px">
        <template slot-scope="scope">
      ...
      <el-table-column label="积分有效期类型" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralLimitTimeType }}</span>
        </template>
      </el-table-column>
      <el-table-column label="积分有效期年限" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralLimitYearNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="积分到期月份" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralEndMonth }}</span>
        </template>
      </el-table-column>
      <el-table-column label="计划积分" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.prepareIntegralNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="计划人数" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.prepareCount }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动描述" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actBreif }}</span>
        </template>
      </el-table-column>
      <el-table-column label="状态: audit_status_tosub：新增，audit_status_wait提交待审核，audit_status_pass审核通过，audit_status_nopass退回" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.state }}</span>
        </template>
      </el-table-column>
      <el-table-column label="所属规则组" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.ruleTeam }}</span>
        </template>
      </el-table-column>
      <el-table-column label="审核意见" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.opinion }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="创建时间" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.crtTime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="最后更新人" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.lstUpdUser }}</span>
        </template>
      </el-table-column>
      <el-table-column label="最后更新时间" min-width="150px">
        <template slot-scope="scope">
