  ...
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="自增主键" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralAdjustId }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="调整类型" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.adjustType }}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  import {
    getList,
    remove
  } from '@/api/system/integraladjust'
  // 按钮的水波纹
