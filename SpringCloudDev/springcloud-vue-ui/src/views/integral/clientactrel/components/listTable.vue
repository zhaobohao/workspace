  ...
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="表id编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.clientActRelId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="客户ID" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.clientId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="积分活动ID" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="账户ID" min-width="150px">
        <template slot-scope="scope">
