  ...
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="自增主键" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralStatusActionId }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="客户名称" min-width="150px">
        <template slot-scope="scope">
      ...
      <el-table-column label="当前余额" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.blance }}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.operationType }}</span>
        </template>
      </el-table-column>
      <el-table-column label="退回原因" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.rejectReason }}</span>
        </template>
      </el-table-column>
      <el-table-column label="状态id" min-width="150px">
        <template slot-scope="scope">
...
<script>
  ...
  import {
    getList,
    remove
  } from '@/api/system/integralstatusaction'
  // 按钮的水波纹
  ...
  export default {
    // TODO:本页面的名称
    name: 'integralstatusaction-listTable',
    components: {
      Pagination
    },
    directives: {
