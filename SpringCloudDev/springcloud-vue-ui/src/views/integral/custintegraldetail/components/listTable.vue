<template>
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="表id主键" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.custIntegralDetailId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="账户号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.accountId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="客户编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.custId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="客户姓名" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.custName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="证件类型" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.identyType }}</span>
        </template>
      </el-table-column>
      <el-table-column label="证件号码" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.identyCard }}</span>
        </template>
      </el-table-column>
      <el-table-column label="客户电话" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.phoneNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="客户地址" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.custAddress }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="积分类型ID" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralTypeId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="积分有效期" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralValidDate }}</span>
        </template>
      </el-table-column>
      <el-table-column label="积分有效期余额" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralValidBlance }}</span>
        </template>
      </el-table-column>
      <el-table-column label="账户余额" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.accountBlance }}</span>
        </template>
      </el-table-column>
      <el-table-column label="已使用总额" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.usedToatl }}</span>
        </template>
      </el-table-column>
      <el-table-column label="账户状态" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.accountStatus }}</span>
        </template>
      </el-table-column>
      <el-table-column label="账户状态ID" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.accountStatusId }}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  import {
    getList,
    remove
  } from '@/api/system/custintegraldetail'
  // 按钮的水波纹
  ...
  export default {
    // TODO:本页面的名称
    name: 'custintegraldetail-listTable',
    components: {
      Pagination
    },
    directives: {
