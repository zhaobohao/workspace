  ...
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="表id编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actRuleInfoId }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="规则计算类型 " min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.type }}</span>
        </template>
      </el-table-column>
      <el-table-column label="优先级" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.grade }}</span>
        </template>
      </el-table-column>
