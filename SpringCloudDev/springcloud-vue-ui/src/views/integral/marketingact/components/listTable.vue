<template>
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="表id" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.marketingActId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动CODE" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动名称" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动开始时间" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.beginTime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动结束时间" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.endTime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="卡券编号" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.cardTieketId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="积分类型" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralType }}</span>
        </template>
      </el-table-column>
      <el-table-column label="积分类型ID" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.integralId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动链接" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actLink }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动状态" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actStatus }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动状态ID" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actStatusId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动说明" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actExplain }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动目标描述" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actTargetBreif }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动卡券数量" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actCardTicketNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动目标类型" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actTargetType }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动目标数量" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actTargetNumNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动积分数量" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.actIntegralNumNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="审核评语" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.reviewComments }}</span>
        </template>
      </el-table-column>
