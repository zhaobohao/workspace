<template>
  ...
  <el-card>
    ...
    'border-bottom': '1px rgb(103, 194, 58) solid'}" :data="list" :height="tableHeight" :stripe="isStripe" border fit
      ...
      <el-table-column label="用户姓名" min-width="150px">
        <template slot-scope="scope">
          {{ scope.row.realName }}
        </template>
      </el-table-column>
      <el-table-column label="所属角色" min-width="150px">
        <template slot-scope="scope">
          {{ scope.row.roleName }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="电子邮箱" min-width="150px">
        <template slot-scope="scope">
          {{ scope.row.email }}
        </template>
      </el-table-column>
      <el-table-column label="用户性别" min-width="150px">
        <template slot-scope="scope">
          {{ scope.row.sex | translateVlaue2Lable(sexOptions) }}
        </template>
      </el-table-column>
    ...
    methods: {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['id', '登录账号', '所属租户', '用户昵称', '用户姓名', '所属角色', '所属部门', '手机号码', '电子邮箱', '用户性别', '用户生日']
          const filterVal = ['id', 'account', 'tenantId', 'name', 'realName', 'roleId',
            'deptId', 'phone', 'email', 'sex', 'birthday'
          ]
