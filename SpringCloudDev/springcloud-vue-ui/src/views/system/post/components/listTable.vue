<template>
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <el-table-column label="岗位类型" min-width="150px" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <span class="link-type"
            click="handleUpdate(scope.row)">{{ scope.row.category | translateVlaue2Lable(postOptions) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="岗位编号" min-width="150px" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.postCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="岗位名称" min-width="150px" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.postName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="岗位排序" min-width="150px" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <span class="link-type" click="handleUpdate(scope.row)">{{ scope.row.sort }}</span>
        </template>
      </el-table-column>
      <el-table-column label="岗位描述" min-width="150px" :show-overflow-tooltip="true">
        <template slot-scope="scope">
