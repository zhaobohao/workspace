<template>
  <el-card>
    ...
    <el-table :key="tableKey" ref="messageTable" v-loading="listLoading" :data="list" :height="tableHeight"
      ...
      <!--表格的序号-->
      <el-table-column :label="$t('table.id')" fixed type="index" width="50"></el-table-column>
      <!--
      ...
      <el-table-column label="应用id" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type" @click="handleUpdate(scope.row)">{{ scope.row.clientId }}</span>
        </template>
      </el-table-column>

      <el-table-column label="应用密钥" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.clientSecret }}</span>
        </template>
      </el-table-column>
      <el-table-column label="授权类型" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.authorizedGrantTypes }}</span>
        </template>
      </el-table-column>
      <el-table-column label="授权范围" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.scope }}</span>
        </template>
      </el-table-column>
      <el-table-column label="令牌秒数" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.accessTokenValidity }}</span>
        </template>
      </el-table-column>
      <el-table-column label="刷新秒数" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.webServerRedirectUri }}</span>
        </template>
      </el-table-column>
      <el-table-column label="回调地址" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.accessTokenValidity }}</span>
        </template>
      </el-table-column>
      <el-table-column label="资源集合" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.resourceIds }}</span>
        </template>
      </el-table-column>
      <el-table-column label="权限" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.authorities }}</span>
        </template>
      </el-table-column>
      <el-table-column label="自动授权" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.autoapprove }}</span>
        </template>
      </el-table-column>
      <el-table-column label="附加说明" width="180px">
        <template slot-scope="scope">
          <span class="">{{ scope.row.additionalInformation }}</span>
        </template>
      </el-table-column>
