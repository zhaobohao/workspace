...
<template>
  <el-card v-show="isSearchCardShow" style="margin-bottom: 20px">
    ...
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.query.account" placeholder="角色名称" style="width: 200px;"
        @keyup.enter.native="roleName" />
      <el-select v-model="listQuery.query.tenantId" placeholder="所属租户" class="filter-item">
        <el-option v-for="item in tenantOptions" :key="parseInt(item.tenantId)" :label="item.tenantName"
          :value="item.tenantId" />
      </el-select>
      <el-input v-model="listQuery.query.roleAlias" placeholder="角色别名" style="width: 200px;"
